...
def main(start_time_str='2024-07-30T00:00:00Z', end_time_str='2024-07-30T23:59:59Z', latitude=51.1950, longitude=0.2757, res_minutes = 1, n_points = 1000, length = 20, radius = 5, xy_angle = 0, z_angle = 0, transmissivity = 1):
    ...
    #tunnel geometry#
    tunnel = Polytunnel(radius=radius, length=length, n_points=n_points, xy_angle=xy_angle, z_angle=z_angle, theta_margin = 0, cell_thickness=5, cell_gap=7)
    ...
    sun_incident = sun.sunvec_tilts_grid(sun_vecs, normals_unit_surface)
    ...
    optical_wavelengths, optical_intensities, spectra_frames = sun.get_spectra(500, 600) #zero tilt
    ...
    #Irradiance#
    transmitted_frames, absorbed_frames = irradiance.transmitted_absorbed_spectra(optical_wavelengths, solar_cell_spectra_shaded)
    ...
    viz.animate_irradiance(time_array, surface_grid_x, surface_grid_y, absorbed_int, "figures/direct-irradiance-surface-animation.mp4")
